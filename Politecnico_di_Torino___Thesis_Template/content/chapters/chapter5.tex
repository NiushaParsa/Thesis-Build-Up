...
The results follow the methodological sequence, beginning with the
fixed benchmark and early adaptive baselines. Classification and
retrieval outcomes are kept separate throughout. The final sections
return to the central comparison: whether the extra decisions improve
evidence retrieval under the stated protocol.
...
The early baseline report used a different uncertainty calculation:
500 question-level bootstrap replicates with seed 13. This thesis uses
the later paper-cluster intervals for the fixed-size table and the
Phase 4 comparisons. The point scores come from the same frozen
outputs, but the two interval procedures must not be conflated.

\section{Fixed granularity and oracle headroom}
\input{content/generated/fixed_results}
...
tokens. The mean of per-chunk maximum evidence similarities moves in the opposite direction,
...
more complex model.
All four logistic candidates and the MLP candidate are retained in
Table~\ref{tab:earlycandidates}, including their different random seeds.
...
\input{content/generated/qwen_results}
...
Table~\ref{tab:qwencheckpoints} reports the intermediate checkpoints
for this run and the subsequent three-epoch variants, so that the
selected endpoints do not hide the training history.
...
\input{content/generated/tree_results}
...
macro F1 0.2247 and mean retrieval F1 0.2717. The mean increase
over the linear tree model is about 0.0040, although median retrieval
F1 falls from 0.2523 to 0.2487. Training-fold macro F1 selects the
full-tree representation, 0.2198 versus 0.2164 for level-only features;
this is not selection on the external validation result.
The level-only XGBoost model has validation macro F1 0.1822.
The weighted nonlinear setup improves some outcomes over the
uniformly weighted linear model, but does not isolate the effect of
nonlinearity alone. Its mean retrieval outcome is still below fixed
...
Table~\ref{tab:secondaryvariants} collects the secondary representation
and heuristic results; none is assigned an unmeasured retrieval score.
...
\input{content/generated/paired_regret}
...
\input{content/generated/local_results}
...
The training-only wording repair produces valid outputs on 173 of the
175 previous failures under its probe. The probe still fails the
100\% validity gate because two failures remain, motivating
...
recorded allocated GPU memory is approximately 9.03 GiB, with
9.60 GiB reserved.
...
the target deserves careful attention. Evidence length is interpretable but
