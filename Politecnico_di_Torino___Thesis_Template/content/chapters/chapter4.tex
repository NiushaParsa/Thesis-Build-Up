...
The methods follow three changes in the research question: whether the
question alone can predict a useful size, whether document similarity
structure improves that decision, and whether changing the target or
the decision unit better matches retrieval. Each family keeps an
explicit fixed-size reference. The order explains the development of
the work; it does not imply that every later method is better.
Prompt-only and added-feature comparisons isolate narrower changes than
the broader model and target redesigns. Their results are interpreted
accordingly, rather than treating every cross-phase difference as a
controlled effect.
...
class imbalance. Unlike Phase 3A's predesignated primary representation,
Phase 3B selects both the configuration and the primary representation
by grouped training-fold macro F1. The full-tree model wins that
comparison and is locked before the validation retrieval evaluation.
The change from Phase 3A includes both nonlinear capacity and class
weighting, so it is not a one-factor test of model capacity.
...
MoG's soft labels favour the two granularities whose retrieved snippets
rank highest by similarity to the training reference
\cite{zhong-etal-2025-mix}; the present objective instead uses the
observed retrieval-F1 regret of all five actions directly.
...
The first version uses deterministic generation with a limit of eight
new tokens. Some responses answer the question instead of giving a
category. The validity gate rejects these outputs without substituting
defaults, so this version has no downstream classifier result.

\subsection{Constrained first-token scoring}
The repaired version tests revised wording on the 175 invalid training
questions, then compares next-token logits only for the single tokens
\texttt{short}, \texttt{medium}, and \texttt{long}, whose IDs are 8412,
25252, and 4670. One forward pass and an argmax produce a valid category.

The prompt requests one lowercase label, not an answer
(Appendix~\ref{app:protocols}). The wording probe uses only known
training failures; constrained scoring subsequently enforces validity
for every required feature row. These are separate operations.

Local examples, labels, tree ranking, and classifier settings remain
unchanged. Because Qwen receives no task-label updates, OOF fine-tuning
is not needed to remove supervised in-sample contamination from this
feature. Repeated development-set use remains a limitation.
