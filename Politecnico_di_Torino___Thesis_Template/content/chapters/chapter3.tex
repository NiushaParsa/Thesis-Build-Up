...
Eligibility requires usable, nonempty highlighted evidence and a
complete five-size retrieval record, with at least one retrieved chunk
at every size. The frozen validation run separately records one
excluded candidate with the reason \texttt{no\_valid\_evidence}; it is
not counted as a zero-scoring question in the 924-question benchmark.
Source-span mapping imposes a further requirement for local training,
as described below, but not for the primary lexical retrieval score.
...
The repository history begins in February with Weaviate ingestion,
JSON exports, and sample retrieval. It then adds filtering of questions
without usable highlighted evidence, parallel paper processing, and
the move to Qdrant with per-stage checkpoints. These steps established
the data pipeline; they did not compare alternative retrievers on a
common benchmark. The fixed-size evaluation pipeline followed, before
the later modelling phases.
...
use locking and atomic replacement. Two separate implementation problems
were addressed: shared JSONL writes were protected by a lock, and a
duplicate increment in the chunk-completion counter was removed.
These measures matter when embedding requests, retrieval, or training
are interrupted.

The June evaluation work added numeric source offsets, per-chunk
evidence metrics, versioned output schemas, and configuration-aware
identifiers for evaluation and oracle records. Persistence and
idempotency checks tested whether repeating a write preserved the same
logical record. These changes made it possible to freeze comparable
outputs without treating a partially written run as a complete experiment.
...
Of the 46 papers without highlighted evidence, 45 had a valid empty
evidence stage and one lacked the corresponding completion markers;
absence of evidence is not automatically an ingestion error.
These are dated observations of the pipeline at that point. Subsequent
complete experiment artifacts determine the populations reported here,
but do not prove that every issue in the wider collection was repaired.
...
diagnostic. The stored summaries distinguish maximum and mean
query-to-chunk similarity; maximum and mean over all chunk--evidence
pairs; and the mean of each retrieved chunk's maximum similarity to an
evidence item. This last quantity is the \enquote{mean evidence
similarity} used in the early comparison report and the similarity
tie-breaker in the legacy oracle. These aggregates are not interchangeable.
The later per-chunk lexical diagnostics also retain each chunk's best
F1 against an individual evidence item; their average is not joined F1.
...
$10^{-6}$, then use mean maximum chunk-to-evidence similarity to break
ties, and finally prefer
...
than silently reassigning the earlier model's targets. The corresponding
legacy training counts are $(460,675,686,343,81)$; forty tokens is the
training-majority class. This label distribution must not be substituted
for the evidence-length distribution below.
