...
\small
\begin{longtable}{@{}p{2cm}p{5.3cm}p{5.8cm}@{}}
\caption{Chronology of the preserved experimental programme.}
...
\bottomrule
\endfoot
Foundation & Text flattening, five chunk levels, embeddings, Qdrant schema & Resumable ingestion and stable identities; legacy storage code is not a second benchmark\\
...
Phase 2C & Base model with five-class head & Removes response parsing; classification improvement over numeric SFT\\
...
Phase 3B & Nonlinear XGBoost on both representations & Full-tree model improves over linear tree model\\
...
\input{content/generated/lr_grid}
...
\input{content/generated/prediction_counts}
...
For the evidence-length Qwen sequence, checkpoint selection uses
validation macro F1, followed by accuracy, weighted F1, balanced
accuracy, lower cross-entropy, and an earlier checkpoint where
needed. Phase 2E adds lower numeric learning rate as a final exact-tie
criterion. Retrieval cannot revise the locked classification winner.

The tree-model grids use paper-grouped training folds. The corrected
fusion base features are OOF, while its meta-level diagnostics are
not claimed to be fully nested. Phase 4 uses fixed inherited
hyperparameters and saves validation decisions before opening the
validation utility targets. Phase 5A and 5B-v2 similarly separate
prediction-time features from gold-overlap evaluation.

These are phase-specific safeguards. The validation split remains
a development split across the programme as a whole.
...
in this thesis. Personal acknowledgements are intentionally omitted;
they should be written by the author rather than inferred from
experiment records.
