#!/usr/bin/env python
"""Phase 2C Qwen Base sequence-classification granularity router.
...
SUPERVISOR_INSTRUCTION = (
    ...
    "one value from: 1 = very short context, 2 = short context, 3 = medium "
    "context, 4 = long context, 5 = very long context. Return only the number"
)
FORMULATION_VERSION = "qwen-phase2c-base-sequence-classifier-v1"
...
class TrainingConfig:
    """Predeclared Phase 2C optimization and model configuration."""
...
def format_classification_example(
    ...
    if len(input_ids) > max_sequence_length:
        raise RuntimeError(
            ...
            f"{max_sequence_length}. Phase 2C never silently truncates input."
        )
...
def ensure_output_root(output_root: Path) -> dict[str, Any]:
    ...
    if output_root.resolve() in forbidden:
        raise RuntimeError(
            "Phase 2C output root must not reuse a completed experiment root"
        )
    ...
    if marker_path.exists():
        ...
        for key, value in expected.items():
            if existing.get(key) != value:
                raise RuntimeError(
                    f"Phase 2C output-root configuration mismatch at {key}"
                )
        return existing
...
def inspect_phase2c(args: argparse.Namespace) -> dict[str, Any]:
...
def _run_id(mode: str) -> str:
    return (
        f"qwen-phase2c-sequence-classifier-{mode}-"
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("Phase 2C training requires a BF16 CUDA GPU")
    if args.mode == "full" and args.max_steps is not None:
        raise ValueError("A full Phase 2C run cannot use --max-steps")
    config = TrainingConfig()
    ...
    if total_steps < 1:
        raise ValueError("Phase 2C requires at least one optimizer step")
    ...
    run_config = {
        **asdict(config),
        ...
        "repository_commit": os.getenv("PHASE2C_REPOSITORY_COMMIT", "unavailable"),
    ...
    if config_path.exists():
        ...
        if existing_config.get("resume_contract_sha256") != resume_contract_sha256:
            raise RuntimeError("Existing Phase 2C run has a different resume contract")
    else:
    ...
    if args.resume is not None:
        ...
        if state.get("resume_contract_sha256") != resume_contract_sha256:
            raise RuntimeError("Checkpoint Phase 2C resume contract mismatch")
    ...
    if trainable_parameters != total_parameters:
        raise RuntimeError("Phase 2C unexpectedly contains frozen parameters")
    writer.add_text(
        "configuration/run",
        "```json\n"
        + json.dumps(
    ...
    if int(state["global_step"]) != total_steps:
        raise RuntimeError(
            f"Phase 2C ended at step {state['global_step']}; expected {total_steps}"
        )
...
def validate_prediction_identity(
    ...
    if len(predictions) != len(frozen):
        raise RuntimeError("Phase 2C predictions and frozen validation lengths differ")
    ids = [row["question_id"] for row in predictions]
    if len(ids) != len(set(ids)):
        raise RuntimeError("Phase 2C validation contains duplicate prediction IDs")
    if ids != [row["question_id"] for row in frozen]:
        raise RuntimeError("Phase 2C predictions do not preserve validation order")
    for prediction, oracle in zip(predictions, frozen):
        for key in ("question_id", "document_id", "question_text", "oracle_label"):
            if prediction[key] != oracle[key]:
                raise RuntimeError(
                    f"Phase 2C prediction differs from frozen data at {key}: "
                    f"{prediction['question_id']}"
                )
        if prediction["selected_checkpoint"] != checkpoint_id:
            raise RuntimeError("Phase 2C prediction checkpoint ID mismatch")
...
def materialize_final_classification(
    ...
    if metrics != best["classification_metrics"]:
        raise RuntimeError("Final Phase 2C metrics do not reproduce selected checkpoint")
...
def final_validation(args: argparse.Namespace) -> dict[str, Any]:
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("Phase 2C final validation requires a BF16 CUDA GPU")
    config = TrainingConfig()
    phase2.set_deterministic_seed(config.seed)
    output_root = Path(args.output_root)
    ensure_output_root(output_root)
    ...
    if (
        ...
        raise RuntimeError("Selected Phase 2C checkpoint fingerprint mismatch")
    device = torch.device("cuda")
    model.to(device)
...
def main() -> None:
    ...
    if args.command == "inspect":
        ...
        raise RuntimeError(f"Unsupported Phase 2C command: {args.command}")
